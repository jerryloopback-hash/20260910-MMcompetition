...
口径(与 P2a_quantile.py + P2b_solve.py 完全一致):
...
# 命令行可覆盖: python P2b_sweep.py 0.79 0.01 0.85   (起点 步长 终点)
if len(sys.argv) == 4:
...
def rolling_quantile_at(point_pred, true_mat, q, is_pv):
    """点预测 + 过去30天同时点残差的经验 q 分位; 与 P2a_quantile.py 同法。"""
    out = np.zeros((334, T))
